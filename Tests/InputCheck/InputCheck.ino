...
void loop() {
  ...
  for (int i = 0; i < PINS_LEN; i++) Serial.print(vals[i]);
  Serial.println();
/*
  if (vals[0] || vals[1] || vals[2] || vals[3]) {
    ...
    if (temp_x * temp_x + temp_y * temp_y <= ul2) {
      ...
      if(abs(x - curr_x) > 0.01 || abs(y - curr_y) > 0.01){
        shoulder.rotate((toDeg(atan(y / x) - acos((u2 + (x * x) + (y * y) - l2) / (2 * upper * sqrt((x * x) + (y * y)))) - PI) - normDeg(angle0 - 180)));
        elbow.rotate(-(toDeg(-acos((u2 + l2 - (x * x) - (y * y)) / (2 * upper * lower))) - normDeg(angle1 - 180)));
      }
    }
  }
*/
